      ...
      {stages.map((s) => {
        ...
        return (
          <li key={`${s.name}-${s.detail}`} className="flex items-start gap-2.5 text-[12px]">
            <span
              className={`mt-px w-[5.5rem] shrink-0 rounded px-1 py-0.5 text-center font-mono text-[10px] font-semibold ${
...
function GroupLabel({ children }: { children: React.ReactNode }) {
  return (
    <li className="bg-ink/40 px-4 py-1 font-mono text-[10px] uppercase tracking-wider text-slate-600">
      {children}
    </li>
  );
}
