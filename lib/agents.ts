...
/** A stable color for anything not in the roster above — a new agent id still looks designed. */
...
export function getAgent(id: string): AgentPersona {
  return AGENTS[id] ?? { id, name: id, role: "Agent", color: colorFor(id), description: "No persona on file yet for this id." };
}
